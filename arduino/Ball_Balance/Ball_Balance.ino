...
float a10[3] = { (d2 - u * sqrt(3.0f)) / 2.0f, (-u - d2 * sqrt(3.0f)) / 2.0f, 0 };
float a20[3] = { -a10[x], a10[y], 0 };
float b10[3] = { (u * sqrt(3.0f) + d2) / 2.0f, (d2 * sqrt(3.0f) - u) / 2.0f, 0 };
float b20[3] = { d2, u, 0 };
...
float r_platform = 121;          // the distance from the center of the platform to the corner of the platform seen from the pixy2 cam
...
float kp = -1e-3;      // proportional constant
...
void PD() {  // calculates the Proportional and Derivative values and moves the servos
  ...
  delta_time = time_f - time_i;  // change in time
  time_i = millis();
  // sets initial time
  deriv[x] = (error[x] - error_prev[x]) / delta_time;  // x component of derivative
  deriv[y] = (error[y] - error_prev[y]) / delta_time;  // y component of derivative
